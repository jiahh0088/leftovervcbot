...
async def on_message(message):
    ...
    if ctx.valid:
        # If it is a command, route it directly here and terminate to avoid double running or skipping
        await bot.process_commands(message)
        return

    # Only add text points if it is completely normal chat conversation
        ...
        if portal_channel_id and after.channel.id == portal_channel_id:
            ...
            if destination_vcs:
                try: await member.move_to(random.choice(destination_vcs))
                    ...
                    try: await member.send("The Departure tried to take you, but administrative powers bound you to the portal.")
