...
def init_db():
    ...
    # Check if voice_seconds column exists (for backwards compatibility migration)
...
async def help_ui(ctx):
    ...
    embed.add_field(name=f"`{prefix}leaderboard` (or `{prefix}lb`)", value="Display individual text & voice rankings.", inline=False)
    ...
    embed.add_field(name=f"`{prefix}postvcinfo` [#channel]", value="Post the casual VC milestones guidelines framework.", inline=False)
...
async def set_vc_roles(ctx, r1: discord.Role, r2: discord.Role, r3: discord.Role, r4: discord.Role, r5: discord.Role):
    """Sets the roles assigned automatically for the 1h, 5h, 25h, 35h, and 55h milestones."""
    config = load_json(CONFIG_FILE)
...
async def post_vc_info(ctx, channel: discord.TextChannel = None):
    """Preset the clean, nonchalant and emoji-free hours guidelines UI into a specific channel."""
    target_channel = channel or ctx.channel
    embed = discord.Embed(
        title="VC Activity Roles",
...
def get_top_users(guild_id, limit=10):
    ...
    cursor.execute("""
        ...
        WHERE guild_id = ? 
        ORDER BY (points + (voice_seconds / 60)) DESC LIMIT ?
    """, (str(guild_id), limit))
...
def is_eligible_for_vc_tracking(state):
    """Evaluates if user state is unmuted & undeafened in a real public VC."""
    return (
...
def generate_user_embed(guild):
    embed = discord.Embed(title=f"{guild.name.upper()} // INDIVIDUAL STANDINGS", color=discord.Color.dark_grey())
    ...
    for i, (u_id, score, voice_seconds) in enumerate(top_users, 1):
        hours = voice_seconds / 3600.0
        lb_text += f"Rank {i:02d} | <@{u_id}>\nChat: {score} pts | Voice: {hours:.2f} hours\n\n"
        
    embed.description = lb_text
    return embed

def generate_fg_embed(guild):
    embed = discord.Embed(title=f"{guild.name.upper()} // GROUP STANDINGS", color=discord.Color.dark_grey())
